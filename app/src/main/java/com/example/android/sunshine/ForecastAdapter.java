...
//      COMPLETED (7) Replace the single TextView with Views to display all of the weather info
        ...
        forecastAdapterViewHolder.dayTextView.setText(dateString);

        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, weatherId);
        forecastAdapterViewHolder.weatherTextView.setText(description);

        /* Read high temperature from the cursor (in degrees celsius) */
        double highInCelsius = mCursor.getDouble(MainActivity.INDEX_WEATHER_MAX_TEMP);
        String max = SunshineWeatherUtils.formatTemperature(mContext,highInCelsius);
        forecastAdapterViewHolder.maxTextView.setText(max);

        /* Read low temperature from the cursor (in degrees celsius) */
        double lowInCelsius = mCursor.getDouble(MainActivity.INDEX_WEATHER_MIN_TEMP);
        String min = SunshineWeatherUtils.formatTemperature(mContext,lowInCelsius);
        forecastAdapterViewHolder.minTextView.setText(min);
    }
...
//          COMPLETED (6) Get references to all new views and delete this line
            ...
            minTextView = (TextView) view.findViewById(R.id.tv_min);
            maxTextView = (TextView) view.findViewById(R.id.tv_max);
